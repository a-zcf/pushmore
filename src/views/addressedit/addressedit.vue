<template>
  <div class="newaddress">
    ...
    <button class="address_but" @click="postAddOrUpadteDdress">确认修改</button>
...
<script>
import { getAddress, addOrUpadteDdress } from "../../api/api";
...
export default {
  ...
  mounted() {
    ...
    this.$getRequest(getAddress, { id: this.id }).then((res) => {
      if (res.data.code === 0) {
        let address = res.data.address;
        this.name = address.name;
        this.phone = address.phone;
        this.adrress = address.address;
        this.valueArea =
